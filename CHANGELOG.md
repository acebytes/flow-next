...
## [flow-next 2.2.0] - 2026-06-23

### Added
- **`/flow-next:fleet` — parallel-worktree orchestrator over ready specs** (fn-67). The human-gated parallel counterpart to `/flow-next:pilot`: a single invocation inspects every open spec, classifies each by the pilot stage table (`work` / `plan-review` / `spec-completion-review` / `make-pr`) plus a new `needs-interview` row for STUB specs, groups conflicting units into safe waves via file-overlap detection (same algorithm as `/flow-next:resolve-pr` Phase 5), asks the user to approve the wave plan, then launches one short-lived worktree per unit with a `fleet-runner` subagent that chains the primary stage skill into `/flow-next:make-pr` and `/flow-next:resolve-pr`. Each worktree's branch + PR + resolve-pr outcome is recorded in a manifest at `.git/flow-next/fleet-manifest.json` (sibling to pilot's strikes ledger, shared across worktrees via the git common dir). Fleet does NOT merge — `/flow-next:land` is reserved for explicit user invocation per the existing skill-license convention; per-worktree chains stop at resolve-pr and the user clicks merge when CI is green.
  - **Classifier reuses pilot's table verbatim** (fn-67.1), with one addition: STUB specs (`**Status: STUB.**` marker or empty Approach + Acceptance) route to a notification queue instead of auto-dispatching `/flow-next:plan`. Pilot's behavior was the right default for single-tick autonomous loops; under polymorphic parallel dispatch a bad auto-plan would spawn many wasteful task specs that compound at PR time, so fleet is more conservative and requires a human interview first.
  - **Fan-out via `Task` subagents** (fn-67.2). The orchestrator dispatches one `fleet-runner` subagent per wave unit in a single message containing parallel `Task` calls. Each fleet-runner runs in fresh context inside its own worktree (cwd persists across Bash calls), executes its primary skill via the `Skill` tool, then chains `make-pr` and a 2-cycle-bounded `resolve-pr` loop. The runner agent has `disallowedTools: Task` so it cannot recurse, and emits a final JSON block the orchestrator parses with `jq -s '.[-1]'` to update the manifest.
  - **Wave grouping by file-overlap avoidance** (fn-67.3). For each unit, fleet derives an expected file set: `work` units parse `## Affected as-built code` from the task spec (convention introduced in the fn-5.7/.8 task rewrites, see openctl PR #8); `plan-review` / `spec-completion-review` / `plan` units claim `.flow/{specs,tasks,epics}/<spec>*`; `make-pr` units are empty (read-only). Greedy wave assignment places a unit in the first wave whose other members all have disjoint file sets, capped at `min(cpu-2, 4)` (configurable via `--max-concurrent`).
  - **Human-gated by design** (fn-67.4). The approval gate in Phase 6 is non-skippable — fleet never creates a worktree or spawns a subagent before the user explicitly approves the wave plan via `AskUserQuestion`. Pilot is the unattended autonomous driver (one spec, one stage per tick, driven by `/loop` or `/goal`); fleet is the explicit "I want to see and approve the whole batch" alternative. Hard guards refuse to run inside Ralph (`FLOW_RALPH`) or pilot (`PILOT_DRY_RUN`); they are alternative drivers and never nest.
  - **Cleanup is opt-in** (fn-67.5). Successful worktrees stay around after PRs are opened + reviews resolved — the user merges when ready, then runs `<worktree-kit-path> cleanup` to prune merged-PR worktrees in one pass. Mirrors the safe-by-default posture: nothing destructive happens without explicit user action.
  - **Docs**: new `commands/flow-next/fleet.md`, `skills/flow-next-fleet/{SKILL.md,workflow.md}`, `agents/fleet-runner.md`. CLAUDE.md "Where to look" entry and root README.md Commands list updated. Codex mirror regenerated (`./scripts/sync-codex.sh`) with the `generate_openai_yaml` call and `REQUIRED_OPENAI_YAML_SKILLS` array entry.

### Notes
- Minor release — fleet is purely additive. No existing skill or flowctl command behavior changes. Pilot's classifier table is reused without modification (fleet adds rows; it does not edit pilot's). The `gh pr merge` ban remains a fleet invariant alongside the existing skill-wide invariant; `/flow-next:land` stays the sole confined exception.

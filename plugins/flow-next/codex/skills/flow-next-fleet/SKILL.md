...
- **Launching before approval.** The `plain-text numbered prompt` approval gate in Phase 6 is mandatory — fleet never creates a worktree or dispatches a subagent before the user explicitly approves the plan.
...
6. **approval** — present the wave plan via `plain-text numbered prompt`. No launches without explicit user approval.

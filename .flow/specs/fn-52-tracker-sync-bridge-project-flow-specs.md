...
- **R16:** Identity / naming alignment between the two schemes (flow's `fn-NN-slug` vs the tracker's `TEAM-NN`, which can't adopt each other's numbering) — each side carries a reference to the other (flow stores the tracker id per R4; the tracker issue carries the flow spec id, e.g. a `flow:fn-NN` label or `[fn-NN]` title prefix) and titles stay 1:1 via the body sync. For tracker-first specs, optionally mint the flow spec id / slug to echo the tracker key (config), matching the manual workunits convention (`wor-2-…`), so the two naming schemes don't drift. [user]/[paraphrase]

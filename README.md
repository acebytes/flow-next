...
[![Flow-next](https://img.shields.io/badge/Flow--next-v1.8.0-green)](CHANGELOG.md)
...
**Grok Build (xAI)?** If flow-next is already installed in Claude Code, Grok Build picks it up automatically — `grok inspect` shows the skills + hook loaded, zero extra setup. (Don't `grok plugin install` the repo — it's a marketplace, not a single plugin.) Multi-agent flows aren't verified on Grok yet; see [`docs/platforms.md`](plugins/flow-next/docs/platforms.md#grok-build-claude-code-compatibility).
...
| Grok Build (xAI) | Loads via Claude Code compatibility (skills + hooks confirmed); multi-agent flows unverified — [details](plugins/flow-next/docs/platforms.md#grok-build-claude-code-compatibility) |
